...
router.post('/signup', (req, res) => {
    ...
    connection.query(query, [user.email], (err, results) => {
        if (!err) {
            if (results.length <= 0) {
                query = "insert into user(userName, contactNumber, email, password, status, role) values(?, ?, ?, ?, 'false', 'user')";
...
router.get('/get', auth.authenticateToken, checkRole.checkRole, (req, res) => {
    let query = "select userId, userName, email, contactNumber, status from user where role='user'";
...
router.patch('/update', auth.authenticateToken, checkRole.checkRole, (req, res) => {
    let user = req.body;
    let query = "update user set status=? where userId=?";
